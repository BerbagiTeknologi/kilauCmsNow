...
$(function(){
  ...
  const listUrl   = "<?php echo e(route('admin.commentArticle.list')); ?>";
  const toggleUrl = id => "<?php echo e(route('admin.commentArticle.toggleStatus', ':id')); ?>".replace(':id', id);
  const deleteUrl = id => "<?php echo e(route('admin.commentArticle.delete',      ':id')); ?>".replace(':id', id);
  ...
  $(document).on('click','.toggle',function(){
      ...
      Swal.fire({title:'Ubah status?',icon:'question',showCancelButton:true}).then(r=>{
          if(r.isConfirmed){
              $.ajax({url:toggleUrl(id),type:'PATCH',data:{_token:'<?php echo e(csrf_token()); ?>'}})
  ...
  $(document).on('click','.delete',function(){
      ...
      Swal.fire({title:'Hapus komentar?',icon:'warning',showCancelButton:true,confirmButtonText:'Ya'}).then(r=>{
          if(r.isConfirmed){
              $.ajax({url:deleteUrl(id),type:'DELETE',data:{_token:'<?php echo e(csrf_token()); ?>'}})
...
<?php echo $__env->make('AdminPage.App.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\xxxxx\Documents\sso-kilau\kilauCms\resources\views\AdminPage\Artikel\Komentar\index.blade.php ENDPATH**/ ?>