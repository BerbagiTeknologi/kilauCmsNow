...
<script>
const base = "<?php echo e(url('admin/article-kilau/kategori-article')); ?>";
...
$('#create-kategori-form').on('submit', function(e){
    e.preventDefault();
    $.post("<?php echo e(url('admin/article-kilau/kategori-article/create')); ?>", $(this).serialize())
      .done(r=>{
...
$(document).on('click','.show-kat',function(){
    $.get("<?php echo e(url('admin/article-kilau/kategori-article/show')); ?>/"+$(this).data('id'), res=>{
...
$(document).on('click','.edit-kat',function(){
    editId=$(this).data('id');
    $.get("<?php echo e(url('admin/article-kilau/kategori-article/show')); ?>/"+editId, res=>{
        $('#edit-id').val(res.id);
...
$('#edit-kategori-form').on('submit',function(e){
    ...
    if(!editId) return;
    $.ajax({
        url : "<?php echo e(url('admin/article-kilau/kategori-article/update')); ?>/"+editId,
        type: 'POST',
...
$(document).on('click','.del-kat',function(){
    ...
    Swal.fire({title:'Hapus kategori?',icon:'warning',showCancelButton:true})
     .then(ok=>{
        if(!ok.isConfirmed) return;
        $.ajax({
            url : "<?php echo e(url('admin/article-kilau/kategori-article/delete')); ?>/"+id,
            type: 'POST',
            data: {_method:'DELETE'},
            success:r=>{
                loadKategori();
...
<?php echo $__env->make('AdminPage.App.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\xxxxx\Documents\sso-kilau\kilauCms\resources\views\AdminPage\Artikel\Kategori\index.blade.php ENDPATH**/ ?>