...
$(function () {
    ...
    const getUrl   = "<?php echo e(route('lp.article.comment.index', $article->slug)); ?>";
    const postUrl  = "<?php echo e(route('lp.article.comment.store', $article->slug)); ?>";
    const likeBase = "<?php echo e(url('/komentar')); ?>/";
    const articleId = <?php echo json_encode($article->id, 15, 512) ?>;
    ...
    $(document).on('click','.like-komentar',function(){
        if (!isAuthenticated){ $('#authModal').modal('show'); return; }
        if ($(this).hasClass('liked')) return;
        const el=$(this),id=el.data('id');
        $.post(likeBase+id+'/like', {_token:'<?php echo e(csrf_token()); ?>'}, res=>{
        ...
        $.post(postUrl, $(this).serialize(), res=>{
            if (res.status){
                if (window.trackKilauEvent) {
                    window.trackKilauEvent('submit_article_comment', {
                        source: 'comment_form',
                        content_type: 'article',
                        content_id: articleId,
                        comment_type: $('input[name="parent_id"]').val() ? 'reply' : 'comment',
                    });
                }
...
<?php $__env->stopPush(); ?>
<?php /**PATH C:\Users\xxxxx\Documents\sso-kilau\kilauCms\resources\views\LandingPageKilau\Article\komentar.blade.php ENDPATH**/ ?>